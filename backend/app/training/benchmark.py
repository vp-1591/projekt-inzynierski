...
def evaluate_response(response_text: str, ground_truth_tags: list):
    ...
    f1_doc = 0.0 if tp_doc == 0 and fp_doc == 0 and fn_doc == 0 else (2 * tp_doc) / (2 * tp_doc + fp_doc + fn_doc)
...
def report_progress(url, value):
    ...
    with contextlib.suppress(BaseException):
        requests.post(f"{url}/training/progress", json={"stage": "evaluation", "value": value}, timeout=1)
