...
def normalize_llm_response(content: str) -> dict:
    ...
    for tag in raw_tags:
        ...
        if tag_upper in VALID_TAGS:
            ...
            if "EMOTIO" in tag_upper:
                cleaned_tags.add("EMOTIONAL_CONTENT")
            elif "CHERRY" in tag_upper:
                cleaned_tags.add("CHERRY_PICKING")
            elif "CLICKBAIT" in tag_upper:
                cleaned_tags.add("MISLEADING_CLICKBAIT")
            elif "QUOTE" in tag_upper:
                cleaned_tags.add("QUOTE_MINING")
            elif "ANECDOT" in tag_upper:
                cleaned_tags.add("ANECDOTE")
            else:
                cleaned_tags.add(tag_upper)
